...
int main(void){
    ...
    ptr = fopen("/Projeto/dataset/mnist/t10k-labels.idx1-ubyte","rb");

    if(ptr==NULL) {
        printf("Error when opening file: %s!", "/Projeto/dataset/mnist/t10k-labels.idx1-ubyte");
        exit(-1);
    }

    fread(d32,sizeof(d32),1,ptr);
    data_32 = to_int(d32, 4);
    printf("sizeof = %ld magic number = %02d%02d%02d%02d  value = %d\n",sizeof(data_b), d32[0],d32[1],d32[2],d32[3], to_int(d32, 4));
    ...
    ptr = fopen("/Projeto/dataset/mnist/t10k-images.idx3-ubyte","rb");
    ...
    if(data_32 == IMAGE_DATASET) { // test image dataset
        ...
        Mat image = Mat(img_ds.n_rows, img_ds.n_columns, CV_8UC1, img_ds.data); // size_t 	step = AUTO_STEP 
